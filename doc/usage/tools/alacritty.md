...
> **Note**: Alacritty theme configuration is generated by the [theme system](../../guides/terminal/theme-system.md) via `src/theme/templates/alacritty.toml`. There is no standalone `src/alacritty/` directory — theme files are written to `~/.config/alacritty/` by the theme switcher.
