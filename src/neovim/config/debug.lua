...
function M.log_work_detection()
	local work_init = require("config.work-init")
	...
	vim.defer_fn(function()
		...
		if #clients == 0 then
			...
			for _, client in ipairs(clients) do
				M.info("LSP", string.format("Active client: %s (id: %d, pid: %s)", 
					client.name, client.id, tostring(client.pid)))
			end
		end
	end, 1000)
end
...
function M.setup_commands()
	...
	vim.api.nvim_create_user_command("DebugLevel", function(opts)
		...
		if M.levels[level] then
			...
			vim.notify("Invalid level. Use: TRACE, DEBUG, INFO, WARN, ERROR, or FATAL", vim.log.levels.ERROR)
		end
	end, { 
		nargs = 1,
		complete = function()
