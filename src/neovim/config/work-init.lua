...
function M.detect_machine_type()
	...
	if hostname:match("%.corp%.google%.com$") or 
	   hostname:match("%.c%.googlers%.com$") or
	   hostname:match("^cloudtop%-") then
		cached_machine_type = "google"
		vim.g.work_machine_type = cached_machine_type
		return cached_machine_type
	elseif hostname:match("DELL%-PRECISION") or
	       hostname:match("%.garmin%.com$") then
...
function M.init()
	...
	local work_init_path = vim.fn.expand("~/.dotfiles/.dotfiles.private/" .. machine_type .. "/init.lua")
