...
function M.setup_custom_menus()
  ...
  local default_menu = {
    ...
    {
      name = "🍿 File Explorer",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.explorer() else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "e",
    },
    ...
    {
      name = "🍿 Dashboard",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.dashboard() else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "D",
      hl = "ExCyan",
    },
    {
      name = "🍿 Scratch Buffer",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.scratch() else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = ".",
      hl = "ExCyan",
    },
    {
    ...
    {
      name = "🍿 Zen Mode",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.zen() else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "Z",
      hl = "ExCyan",
    },
    {
      name = "🍿 LazyGit",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.lazygit() else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "G",
      hl = "ExCyan",
    },
    ...
    {
      name = "🍿 Terminal",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.terminal() else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "t",
    },
  ...
  local latex_menu = {
    ...
    {
      name = "📐 Insert Subsection",
      cmd = "normal i\\subsection{}",
      rtxt = "lsS",
    },
    {
      name = "🖼️ Insert Figure",
      cmd = "lua vim.api.nvim_put({'\\\\begin{figure}[htbp]', '    \\\\centering', '    \\\\includegraphics[width=0.8\\\\textwidth]{filename}', '    \\\\caption{Caption}', '    \\\\label{fig:label}', '\\\\end{figure}'}, 'l', true, true)",
      rtxt = "lff",
    },
    {
      name = "📊 Insert Table",
      cmd = "lua vim.api.nvim_put({'\\\\begin{table}[htbp]', '    \\\\centering', '    \\\\begin{tabular}{|c|c|}', '        \\\\hline', '        Header1 & Header2 \\\\\\\\', '        \\\\hline', '        Data1 & Data2 \\\\\\\\', '        \\\\hline', '    \\\\end{tabular}', '    \\\\caption{Caption}', '    \\\\label{tab:label}', '\\\\end{table}'}, 'l', true, true)",
      rtxt = "lft",
    },
...
function M.open_file_menu()
  ...
  for _, item in ipairs(M.menus.default) do
    if
      item.name and (item.name:match("File") or item.name:match("Find") or item.name:match("Save"))
    then
...
function M.open_code_menu()
  ...
  for _, item in ipairs(M.menus.default) do
    if
      ...
      and (
        item.name:match("LSP")
        or item.name:match("Definition")
        or item.name:match("References")
        or item.name:match("Format")
      )
    then
...
function M.open_debug_menu()
  ...
  local debug_menu = {
    ...
    },
    {
      ...
      cmd = "lua require('dap').set_breakpoint(vim.fn.input('Condition: '))",
      rtxt = "dB",
    },
    {
      name = "📝 Log Point",
      cmd = "lua require('dap').set_breakpoint(nil, nil, vim.fn.input('Log message: '))",
      rtxt = "dlp",
    },
    {
    ...
    {
      name = "🖥️ Toggle DAP UI",
      cmd = "lua require('dapui').toggle()",
      rtxt = "du",
    },
    {
      name = "📊 Open REPL",
      cmd = "lua require('dap').repl.open()",
      rtxt = "dR",
    },
    {
      name = "🔍 Hover Variables",
      cmd = "lua require('dap.ui.widgets').hover()",
      rtxt = "dh",
    },
    {
      name = "👁️ Toggle Virtual Text",
      cmd = "lua require('nvim-dap-virtual-text').toggle()",
      rtxt = "dvt",
    },
...

    {
      ...
      cmd = "lua vim.ui.input({prompt='Expression: '}, function(expr) if expr then require('dap').eval(expr) end end)",
      rtxt = "dE",
    },
    {
      name = "📖 Open Scopes",
      cmd = "lua local widgets=require('dap.ui.widgets'); widgets.sidebar(widgets.scopes).open()",
      rtxt = "dS",
    },
    {
      name = "🗂️ Open Frames",
      cmd = "lua local widgets=require('dap.ui.widgets'); widgets.sidebar(widgets.frames).open()",
      rtxt = "dF",
    },
  }
...
function M.open_file_management_menu()
  ...
  local file_menu = {
    ...
    {
      name = "🪟 Open Explorer (Float)",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.explorer({ float = true }) else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "O",
    },
    {
    ...
    {
      name = "❌ Close Buffer",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.bufdelete() else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "c",
    },
    ...
    {
      name = "📋 Copy File Path",
      cmd = "lua vim.fn.setreg('+', vim.fn.expand('%:p'))",
      rtxt = "p",
    },
    {
      name = "📋 Copy File Name",
      cmd = "lua vim.fn.setreg('+', vim.fn.expand('%:t'))",
      rtxt = "t",
    },
    {
      name = "📂 Open File Directory",
      cmd = "lua local ok, s = pcall(require, 'snacks'); if ok and s then s.explorer({ cwd = vim.fn.expand('%:p:h') }) else vim.notify('Snacks not loaded', vim.log.levels.WARN) end",
      rtxt = "d",
    },
  }
