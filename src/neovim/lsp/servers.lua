--------------------------------------------------------------------------------
-- lsp/servers.lua - Language Server Protocol configuration
--
-- DESCRIPTION:
...
local function setup_lsp()
  ...
  if vim.fn.filereadable(override_path) == 1 then
    ...
    if ok and override and override.setup then
      if logging then
        logging.info("LSP", "Attempting work-specific LSP setup")
      end
      ...
      if setup_ok then
        ...
        if result == true then
          if logging then
            logging.info("LSP", "Work-specific LSP configuration active")
          end
    ...
    elseif not ok then
      if logging then
        logging.error("LSP", "Failed to load override: " .. tostring(override))
      end
    end
  else
    if logging then
