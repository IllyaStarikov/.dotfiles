...
update_languages() {
  ...
  if has_command npm; then
    ...
    if [[ "${DRY_RUN}" == true ]]; then
        ...
        # Skip npm self-update if managed by Homebrew (causes EEXIST errors)
        if [[ "$(which npm)" != /opt/homebrew/* ]]; then
  ...
  if has_command gem; then
    ...
    if [[ "$(which ruby)" == "/usr/bin/ruby" ]]; then
      ...
      if [[ "${DRY_RUN}" == false ]]; then
        update_with_fallback "gem update" gem update
...
update_tools() {
  ...
  if [[ -d "${dotfiles_dir}/.git" ]]; then
    ...
    if [[ "${DRY_RUN}" == false ]]; then
        update_with_fallback "dotfiles" sh -c "cd '$dotfiles_dir' && git pull --rebase"
    fi
  fi
