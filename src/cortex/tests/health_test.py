...
class TestHealthMonitor(unittest.TestCase):
    ...
    def test_check_mlx_server_running(self):
        """Test MLX server check when running."""
        response = make_response(200, {"data": [{"id": "model1"}, {"id": "model2"}]})
        mock_session = MagicMock()
        mock_session.get = MagicMock(return_value=make_cm(response))

        with patch("aiohttp.ClientSession", make_session_class(mock_session)):
            result = asyncio.run(self.monitor.check_mlx_server())

        self.assertEqual(result["status"], "healthy")
        self.assertTrue(result["running"])
        self.assertEqual(result["models"], 2)

    def test_check_mlx_server_offline(self):
    ...
    def test_check_network_healthy(self):
        ...
        mock_session = MagicMock()
        mock_session.head = MagicMock(return_value=make_cm(make_response(200)))

        with patch("aiohttp.ClientSession", make_session_class(mock_session)):
            result = asyncio.run(self.monitor.check_network())

        self.assertEqual(result["status"], "healthy")
        self.assertEqual(result["connectivity"], 100.0)

    def test_check_network_degraded(self):
        """Test network check with mixed endpoint results."""
        mock_session = MagicMock()
        mock_session.head = MagicMock(
            side_effect=[
                make_cm(make_response(200)),
        ...
        self.assertLess(result["connectivity"], 100)
