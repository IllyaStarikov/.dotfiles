...
class Cortex:
    ...
    async def list_models(
        ...
        if recommended:
            filtered = ModelRecommender.recommend_models(
                self.system_info, filtered, max_recommendations=10
            )
    ...
    async def set_model(self, model_id: str, provider: Optional[str] = None) -> bool:
        ...
        model_info = {
            ...
            "capability": [c.value for c in model.capabilities][0] if model.capabilities else "chat",
            "online": model.online,
