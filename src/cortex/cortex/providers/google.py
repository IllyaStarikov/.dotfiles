...
class GoogleProvider(BaseProvider):
    ...
    FALLBACK_MODELS = [
        "gemini-2.0-flash-exp", "gemini-1.5-pro-latest", "gemini-1.5-flash-latest", "gemini-1.0-pro"
    ]
    ...
    async def fetch_models(self, force_refresh: bool = False) -> List[ModelInfo]:
                        ...
                        if response.status == 200:
                            ...
                            for model_data in api_models:
                                ...
                                if "gemini" in model_id.lower():
                                    ...
                                    known = next((m for m in known_models if m["id"] == model_id),
                                                 None)
                                    if known:
                                        metadata = self._infer_model_metadata(
                                            model_id, {
                                                **model_data, "context": known["context"],
                                                "output": known["output"],
                                                "tier": known["tier"]
                                            })
                                    else:
        ...
        for known in known_models:
            if known["id"] not in existing_ids:
                ...
                model_info = ModelInfo(id=known["id"],
                                       name=known["name"],
                                       provider="gemini",
                                       size_gb=0,
                                       ram_gb=0,
    ...
    def _infer_model_metadata(self, model_id: str, model_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not metadata["description"]:
            if metadata.get("thinking"):
                ...
                metadata[
                    "description"] = f"Model with {metadata['context']//1000000}M token context"
