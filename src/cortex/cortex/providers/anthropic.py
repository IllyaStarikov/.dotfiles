...
class AnthropicProvider(BaseProvider):
    ...
    async def fetch_models(self, force_refresh: bool = False) -> List[ModelInfo]:
        ...
        known_models = [
            # Claude 3.5 Models
            {"id": "claude-3-5-sonnet-20241022", "name": "Claude 3.5 Sonnet (Oct 2024)", "tier": "sonnet", "context": 200000, "output": 8192},
            {"id": "claude-3-5-haiku-20241022", "name": "Claude 3.5 Haiku (Oct 2024)", "tier": "haiku", "context": 200000, "output": 8192},

            # Claude 3 Opus
            {"id": "claude-3-opus-20240229", "name": "Claude 3 Opus", "tier": "opus", "context": 200000, "output": 4096},

            # Claude 3 Sonnet
            {"id": "claude-3-sonnet-20240229", "name": "Claude 3 Sonnet", "tier": "sonnet", "context": 200000, "output": 4096},

            # Claude 3 Haiku
            {"id": "claude-3-haiku-20240307", "name": "Claude 3 Haiku", "tier": "haiku", "context": 200000, "output": 4096},

            # Claude 2 Models (Legacy)
            {"id": "claude-2.1", "name": "Claude 2.1", "tier": "legacy", "context": 200000, "output": 4096},
            {"id": "claude-2.0", "name": "Claude 2.0", "tier": "legacy", "context": 100000, "output": 4096},

            # Claude Instant (Legacy)
            {"id": "claude-instant-1.2", "name": "Claude Instant 1.2", "tier": "instant", "context": 100000, "output": 4096},
        ]
    ...
    async def _probe_model_variants(self, session, headers, base_model_id: str, models: List[ModelInfo]):
        """Probe for dated variants of a model."""
        # Common date patterns to try
        date_patterns = [
            "20250805", "20250514", "20250219",  # 2025 dates
            "20241022", "20240620", "20240307", "20240229",  # 2024 dates
        ]
    ...
    def _create_model_info(self, model_id: str, api_data: Dict[str, Any]) -> ModelInfo:
        ...
        if "claude-4" in model_id.lower() or "opus-4" in model_id.lower() or "sonnet-4" in model_id.lower():
