...
class OllamaProvider(BaseProvider):
    ...
    async def _get_model_details(self, session: aiohttp.ClientSession, model_data: Dict[str, Any]) -> Optional[ModelInfo]:
        ...
        try:
            ...
            async with session.post(f"{self.api_url}/show", json={"name": model_name}) as response:
                if response.status == 200:
                    ...
                    description = f"{family} {param_size} model" if family else f"Ollama model {param_size}"
                    if quant:
    ...
    def _extract_capabilities_from_api(self, details: Dict[str, Any], model_name: str) -> List[ModelCapability]:
        """Extract capabilities from API response."""
        capabilities = []
