...
class MLXProvider(BaseProvider):
                ...
                async with session.get(self.MLX_HUB_API, params=params) as response:
                    if response.status == 200:
                        ...
                        for model_data in data:
                            ...
                            if not (
                                model_id.startswith('mlx-community/')
                                or 'mlx' in model_data.get('library_name', '')
                            ):
                                continue
    ...
    async def _parse_huggingface_model(
        ...
        try:
            ...
            return ModelInfo(
                ...
                open_source=True,
                recommended_ram=(
                    size_gb if size_gb > 0 else self._extract_size_from_name(model_name)
                )
                * 1.2,
                description=description[:200]
                if description
                else 'MLX model optimized for Apple Silicon',
                score=score,
                downloads=downloads,
                likes=likes,
                metadata={
    ...
    def _extract_capabilities(
        self, model_name: str, tags: List[str], model_details: Dict[str, Any]
    ...
    async def download_model(self, model_id: str, progress_callback=None) -> bool:
        ...
        try:
            ...
            check_cmd = await asyncio.create_subprocess_exec(
                'which', 'mlx_lm', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
