...
class MLXProvider(BaseProvider):
                ...
                async with session.get(self.MLX_HUB_API, params=params) as response:
                    if response.status == 200:
                        ...
                        for model_data in data:
                            ...
                            if not (model_id.startswith("mlx-community/") or "mlx" in model_data.get("library_name", "")):
                                continue
    ...
    async def _parse_huggingface_model(self, session: aiohttp.ClientSession, model_data: Dict[str, Any]) -> Optional[ModelInfo]:
        ...
        try:
            ...
            return ModelInfo(
                ...
                online=False,
                open_source=True,
                recommended_ram=(size_gb if size_gb > 0 else self._extract_size_from_name(model_name)) * 1.2,
                description=description[:200] if description else f"MLX model optimized for Apple Silicon",
                score=score,
                downloads=downloads,
                likes=likes,
                metadata={
    ...
    def _extract_capabilities(self, model_name: str, tags: List[str], model_details: Dict[str, Any]) -> List[ModelCapability]:
        ...
        if any(x in model_lower for x in ["code", "coder", "codellama", "deepseek-coder", "qwen2.5-coder"]):
    ...
    async def download_model(self, model_id: str, progress_callback=None) -> bool:
        ...
        try:
            ...
            is_quantized = any(x in model_id.lower() for x in ["-4bit", "-8bit", "-2bit", "-3bit", "-6bit"])
            ...
            if is_quantized:
                ...
                cmd = ["mlx_lm", "convert", "--hf-path", model_id, "--mlx-path", str(output_path), "-q"]
            ...
            if check_cmd.returncode != 0:
                ...
                if is_quantized:
                    cmd = ["python", "-m", "mlx_lm.convert", "--hf-path", model_id, "--mlx-path", str(output_path)]
                else:
                    cmd = ["python", "-m", "mlx_lm.convert", "--hf-path", model_id, "--mlx-path", str(output_path), "-q"]
