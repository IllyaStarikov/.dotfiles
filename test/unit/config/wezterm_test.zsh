...
# theme.lua is GENERATED by the theme system and gitignored - absent on fresh
# checkouts and CI; validate it only when present.)

test_case "theme.lua is valid when generated"
if [[ ! -f "$DOTFILES_DIR/src/wezterm/theme.lua" ]]; then
  skip "src/wezterm/theme.lua not generated yet (run switch-theme.sh)"
elif ! command -v luac >/dev/null 2>&1; then
  skip "luac not available"
elif luac -p "$DOTFILES_DIR/src/wezterm/theme.lua" 2>/dev/null; then
  pass
else
